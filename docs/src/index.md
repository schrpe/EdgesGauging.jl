...
- **Profile extraction along arbitrary paths** — sample an image along a line
  segment or a circular arc with selectable interpolation
  ([`INTERP_NEAREST`](@ref InterpolationMode), [`INTERP_BILINEAR`](@ref InterpolationMode),
  [`INTERP_BICUBIC`](@ref InterpolationMode)), configurable strip width, and
  explicit `NaN` for out-of-bounds samples.
- **Robust geometric fitting** via a generic RANSAC engine ([`ransac`](@ref),
  [`ransac2`](@ref)) with constraint support — see [`LineConstraints`](@ref)
  and [`CircleConstraints`](@ref) for angle, radius, arc completeness, and
  inlier-count limits.
...
Using the test image `test/blob.tif` — a 480×640 grayscale photograph of a
metal plate — we demonstrate [`gauge_line`](@ref) on the slanted right edge
and [`gauge_circle`](@ref) on the upper round hole.
...
![Line and circle gauging on test/blob.tif](blob_example.png)

## Profile extraction along arbitrary paths

[`extract_line_profile`](@ref) and [`extract_arc_profile`](@ref) sample image
intensity along a straight segment or a circular arc, returning a 1-D
`Vector{Float64}` ready to feed into [`gauge_edges_in_profile`](@ref):
...
Out-of-bounds samples are emitted as `NaN`. [`gauge_edges_in_profile`](@ref)
is NaN-tolerant: NaN-padded profiles flow straight through and the edge is
...
[`gauge_line`](@ref) and [`gauge_circle`](@ref) throw [`GaugingError`](@ref)
with a `reason::Symbol` field when a pipeline fails:
